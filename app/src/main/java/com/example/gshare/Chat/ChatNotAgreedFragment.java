                ...
                handler.postDelayed(new Runnable() {
                    ...
                    public void run() {
                        ...
                        if (user.equals(itemOwner)) {
                            if (notice.getNoticeType() == Notice.BORROW_NOTICE) {
                                notice.setTempG(Integer.parseInt(editG.getText().toString()));
                                notice.setTempDay(Integer.parseInt(editDay.getText().toString()));
                            } else {
                                try {
                                    Notice notice2 = new Notice("", 2, "", 2, null, 0, null, "");
                                    notice2.setG(notice.getG());
                                    notice2.setChatG(Integer.parseInt(editG.getText().toString()));
                                    notice.setTempG(Integer.parseInt(editG.getText().toString()));
                                } catch (IllegalArgumentException e) {
                                    Toast.makeText(getActivity(), "You can't make g value higher", Toast.LENGTH_LONG).show();
                                    notice.setTempG(notice.getG());
                                }
                                notice.setTempDay(Integer.parseInt(editDay.getText().toString()));
                            }
