...
public class CreateYellowNoticeFragment extends Fragment implements View.OnClickListener , AdapterView.OnItemClickListener {
        ...
        if (v.getId() == R.id.PublishButtonYellow) {
            try {
                notice = new Notice(itemName.getText().toString(), Integer.parseInt(day.getText().toString()), note.getText().toString(), category,
                        currentUser,Integer.parseInt(gValue.getText().toString()), new LocationG(),"");
                try {
                    addNotice( notice );
                } catch (Exception e) {
                    e.printStackTrace();
